    ...
    <div class="w-full min-h-screen bg-gray-50 flex flex-col">
        <main class="flex-grow p-8">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-semibold text-gray-800">Edit Page</h1>
                    <p class="text-gray-500">Update the content and visibility of this page.</p>
                </div>
            ...
            <div class="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-100">
                <div class="p-6 md:p-8">
                    ...
                    <form method="POST" action="{{ route('admin.page.update', $page->id) }}" class="space-y-8">
                        @csrf
                        @method('PUT')

                        <div class="grid gap-6 md:grid-cols-2">
                            {{-- Page Name --}}
                            <x-admin.input label="Page Name" name="name" :value="old('name', $page->name)" required icon="fas fa-file"
                                placeholder="Page name" data-slug-field="name"
                                data-slug-route="{{ route('admin.page.getslug') }}" />

                            {{-- Slug --}}
                            <x-admin.input label="Slug" name="slug" :value="old('slug', $page->slug)" required icon="fas fa-link"
                            ...
                            {{-- Navbar --}}
                            <x-admin.select label="Show in Navbar" name="navbar" :options="[0 => 'No', 1 => 'Yes']" :value="old('navbar', $page->navbar)"
                                icon="bars" />

                            {{-- Footer --}}
                            <x-admin.select label="Show in Footer" name="footer" :options="[0 => 'No', 1 => 'Yes']" :value="old('footer', $page->footer)"
                                icon="shoe-prints" />
                        </div>
