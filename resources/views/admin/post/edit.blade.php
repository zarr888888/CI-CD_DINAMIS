        ...
        <main class="flex-grow p-8">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p class="text-gray-500">Update your post and republish it.</p>
                </div>
                ...
                <div class="p-6 md:p-8">
                    ...
                    <form method="POST" action="{{ route('admin.post.update', $post) }}" enctype="multipart/form-data"
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            {{-- Title / Slug --}}
                            <x-admin.input label="Title" name="title" :value="old('title', $post->title)"
                                placeholder="Title of the post" icon="heading" required data-slug-field="title"
                            ...
                            {{-- Category --}}
                            <x-admin.select name="category_id" label="Category" :options="$categories" :value="old('category_id', $post->category_id)"
                                placeholder="— Select category —" icon="fas fa-sitemap" />

                            {{-- Published --}}
                            <x-admin.select name="status" label="Published" :options="[1 => 'Yes', 0 => 'No']" :value="old('status', (int) $post->status)"
                            ...
                            <div>
                                ...
                                <select id="tags" name="tags[]" multiple size="6"
                                    ...
                                    @foreach ($tags as $id => $label)
                                        <option value="{{ $id }}" @selected(in_array($id, (array) old('tags', $post->tags->pluck('id')->toArray())))>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        {{-- Content (Jodit) --}}
                        <div>
                            <label for="editor" class="block mb-2 text-sm font-medium text-gray-900">
                                Content
                            </label>
            ...
            <div class="mt-6 text-xs text-gray-500">
                ...
                You can adjust the slug or keep it as-is.
            </div>
        </main>
    </div>
